...
function scoreLabel(score?: number) {
  ...
  return `${Math.max(0, Math.min(100, normalized)).toFixed(normalized >= 10 ? 0 : 1)}%`;
}
...
export function EvidenceInspector({ citations = [], retrieval, lang }: Props) {
  ...
  return (
    <section className="evidenceInspector" data-no-ui-translate>
      {citations.length > 0 && (
        <details className="evidenceSection" open>
          <summary>
                ...
                <article className="evidenceCard" key={`${source.url || source.title}-${index}`}>
                  ...
                  <div className="evidenceBody">
                    <div className="evidenceTitleRow">
                      ...
                      {score && <span className="evidenceScore" title={en ? "Similarity score returned by the knowledge-base API" : "知识库 API 返回的相似度分数"}>{score}</span>}
                    </div>
        ...
        <details className="evidenceSection retrievalSection">
          <summary>
            ...
            <span>
              ...
              <small>{en ? `${retrieval.retrievedCount} candidates inspected · ${retrieval.usedCount} answer sources` : `检查 ${retrieval.retrievedCount} 个候选片段 · 回答引用 ${retrieval.usedCount} 个来源`}</small>
            </span>
            <b>{en ? "Details" : "详情"}</b>
          </summary>
          ...
          <div className="retrievalBody">
            <div className="retrievalPipeline" aria-label={en ? "Retrieval pipeline" : "检索流程"}>
              ...
              <span>{en ? "Vector search" : "向量检索"}</span><i>→</i>
              <span>Top {retrieval.topN}</span><i>→</i>
              <span>{en ? "Answer sources" : "回答来源"} {retrieval.usedCount}</span>
            </div>
            ...
            <dl className="retrievalFacts">
              ...
              <div><dt>{en ? "Score threshold" : "相似度阈值"}</dt><dd>{retrieval.threshold}</dd></div>
            </dl>

            <p className="retrievalNote">{en
              ? "The list below is an independent vector-search diagnostic used to inspect retrieval quality. The Source evidence section above shows the sources actually returned with the AnythingLLM answer."
              : "下方列表是独立向量检索诊断，用于检查召回质量；上方“来源证据”才是 AnythingLLM 随本次回答实际返回的引用来源。"}</p>
