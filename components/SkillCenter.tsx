  ...
  return <>
    <aside className={`skillRail ${collapsed ? "skillRailCollapsed" : ""}`}>
      {collapsed ? <div className="skillRailMini">
        <button type="button" className="skillExpandButton" onClick={toggleCollapsed} title="展开技能中心" aria-label="展开技能中心">‹</button>
        <span className="skillMiniGlyph">✦</span>
        <strong>技能</strong>
        {activeName && <i title={`已选择：${activeName}`} />}
      </div> : <>
        <div className="skillRailHeader">
          <div><span>SKILL CENTER</span><h2>技能中心</h2></div>
          <div className="skillHeaderActions"><button type="button" onClick={clearAll}>清除</button><button type="button" className="skillCollapseButton" onClick={toggleCollapsed} title="收起技能中心" aria-label="收起技能中心">›</button></div>
        </div>
    ...
    <style jsx global>{`
      ...
      .dashboardShell:has(.skillRailCollapsed){grid-template-columns:205px minmax(0,1fr) 56px!important}
      .skillRail{transition:width .2s ease,padding .2s ease;overflow:hidden}.skillRailCollapsed{width:56px!important;min-width:56px!important;padding:0!important;background:#fff;border-left:1px solid #e7eaf0}.skillRailMini{height:100%;display:flex;flex-direction:column;align-items:center;padding-top:16px;gap:12px}.skillRailMini strong{writing-mode:vertical-rl;letter-spacing:.12em;font-size:11px;color:#6873d8}.skillRailMini i{width:8px;height:8px;border-radius:50%;background:#665ee8;box-shadow:0 0 0 4px #efedff}.skillMiniGlyph{width:32px;height:32px;border-radius:10px;background:#efedff;color:#665ee8;display:grid;place-items:center;font-size:16px}.skillExpandButton,.skillCollapseButton{border:1px solid #e1e5eb!important;background:#fff!important;color:#6b7480!important;width:30px;height:30px;border-radius:9px!important;padding:0!important;font-size:20px;line-height:1;cursor:pointer}.skillHeaderActions{display:flex;gap:6px;align-items:center}
      ...
      @media(max-width:1050px){.dashboardShell:has(.skillRailCollapsed){grid-template-columns:180px minmax(0,1fr) 52px!important}.skillRailCollapsed{width:52px!important;min-width:52px!important}}
    `}</style>
  </>;
}
