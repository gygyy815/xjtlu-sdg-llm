...
type SourceStatus = {
  ...
  approved?: boolean;
  policy?: "source+all" | "source-only" | "all-only" | "pending_workspace";
  targets?: string[];
  reason?: string | null;
};
type SyncStatus = {
...
function sourceLabel(source: SourceStatus) {
  if (source.account === "未分类") return "需补来源";
  if (source.policy === "all-only") return "仅进入总库";
  if (source.policy === "source+all") return "来源库 + 总库";
  if (source.policy === "source-only") return "仅来源库";
  if (source.approved && !source.workspaceExists) return "Workspace 不存在";
  return "待新增映射";
}
...
export default function KnowledgeBasePage() {
  ...
  const pendingSources = phase2Sources.filter((source) => source.policy === "pending_workspace" || source.account === "未分类");
  ...
  return <main className="kbPage">
    ...
    <section className="kbHero">
      ...
      <p>AnythingLLM Workspace 负责检索；服务器文章仓库负责新增、更新、去重与来源识别。Phase 2 不再自动创建 Workspace；新来源可以选择建立独立 Workspace，或在允许时仅进入跨公众号总库。</p>
      <div className="kbActions"><button type="button" onClick={load}>刷新状态</button><Link href="/dashboard">查看数据看板 →</Link></div>
    </section>

    <section className="metricGrid">
      <article><small>Demo Workspace</small><strong>{loading ? "…" : workspaces.length}</strong><span>只控制前端下拉框显示</span></article>
      ...
      <article><small>待确认公众号</small><strong>{sync.connected ? pendingSources.length : "—"}</strong><span>未分类、未映射或映射失效</span></article>
    </section>

    <section className="kbPanel">
      <div className="panelHead"><div><span>ANYTHINGLLM</span><h2>Demo 中可选的正式知识库</h2></div><strong>{workspaces.length} 个</strong></div>
    ...
    <section className="kbPanel">
      <div className="panelHead"><div><span>SOURCE REGISTRY</span><h2>公众号来源识别与 Workspace 策略</h2></div><strong>{phase2Sources.length || sync.accounts?.length || 0} 个来源</strong></div>
      <div className="sourceRule">
        <strong>所有公众号文章可以放在同一个 incoming 文件夹。</strong>
        <p>系统优先读取文章里的 <code>source_account</code>；没有该字段时才使用 <code>incoming/&lt;公众号&gt;/文件</code> 的文件夹名；两者都没有就标记“未分类”。来源身份写进 SQLite 和 processed 文档，所以物理文件是否混放不影响后续区分。</p>
      </div>

      {!sync.connected ? <div className="empty">服务器状态尚未连接，部署到 ECS 后这里会自动列出检测到的公众号。</div> : phase2Sources.length ? <div className="sourceTable">
        <div className="sourceRow head"><span>公众号</span><span>文章</span><span>独立 Workspace</span><span>同步策略</span></div>
        {phase2Sources.map((source) => <div className="sourceRow" key={source.account}>
          <strong>{source.account}</strong>
          <span>{source.count}</span>
          <code>{source.workspace || "—"}</code>
          <i className={source.policy && source.policy !== "pending_workspace" && source.account !== "未分类" ? "ok" : "pending"}>{sourceLabel(source)}</i>
        </div>)}
      ...
      <div className="howToAdd">
        <h3>遇到新的公众号有两种方式</h3>
        <ol>
          <li><strong>需要单独检索：</strong>在 AnythingLLM 手动创建 Workspace，再把 <code>&quot;公众号名&quot;:&quot;workspace-slug&quot;</code> 加入 <code>XJTLU_SOURCE_WORKSPACES</code>。</li>
          <li><strong>只需要进入总库：</strong>配置 <code>ANYTHINGLLM_ALL_WORKSPACE_SLUG</code>，并将 <code>XJTLU_SYNC_UNMAPPED_TO_ALL=true</code>。只要文章有明确 <code>source_account</code>，新公众号无需新建 Workspace，也可以进入总库。</li>
          <li><code>ANYTHINGLLM_WORKSPACES</code> 只决定 Demo 顶部下拉框显示哪些知识库，不必把几十个公众号全部放进去。</li>
          <li>任何真实同步前都先运行 <code>npm run sync:anythingllm:discover</code> 和 <code>npm run sync:anythingllm:dry-run</code>。</li>
        </ol>
      </div>

      {sync.phase2?.allWorkspace && <div className={`notice ${sync.phase2.allWorkspaceExists ? "success" : "warning"}`}><strong>跨公众号总库：</strong> {sync.phase2.allWorkspace} · {sync.phase2.allWorkspaceExists ? `存在。未映射来源${sync.phase2.allowUnmappedToAll ? "允许仅进入总库" : "仍会等待确认"}。` : "当前 AnythingLLM 中未找到该总库。"}</div>}
    ...
    <section className="kbPanel">
      ...
      <div className="pipeline"><b>processed</b><i>→</i><b>来源识别</b><i>→</i><b>同步策略</b><i>→</i><b>AnythingLLM raw-text API</b><i>→</i><b className="future">synced</b></div>
