...
export default function MindMapPage() {
  ...
  return <main className="toolPage">
    ...
    {map && <section className="mindCard">
      <div className="mindHead"><div><span>INTERACTIVE MARKMAP</span><h2 data-no-ui-translate>{map.title}</h2><p>{map.summary}</p></div><div className="mindActions">
    ...
    {map && <section className="sourcePanel"><h2>参考来源</h2>{citations.length ? citations.map((item, index) => <article key={index} data-no-ui-translate><strong>S{index + 1}. {item.title}</strong>{item.source && <span>{item.source}</span>}{item.publishedDate && <span>{item.publishedDate}</span>}{item.text && <p>{item.text.slice(0, 260)}</p>}{item.url && <a href={item.url} target="_blank" rel="noreferrer">查看原文 ↗</a>}</article>) : <div className="sourceEmpty">本次检索没有返回可展示来源。建议确认 Workspace 文档是否包含来源元数据，或稍后重试。</div>}</section>}
