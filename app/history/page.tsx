...
export default function HistoryPage() {
  ...
  return <main className="historyPage">
    ...
    <section className="historyHero">
      <span>我的对话历史</span>
      <h1>只显示当前用户在本浏览器产生的 Demo 对话</h1>
      <p>这一版不再枚举 AnythingLLM 中所有 Workspace 的全局聊天记录。Demo 会记住当前浏览器最近使用的 API sessionId，并只按这些 sessionId 向 AnythingLLM 查询历史，因此其他测试用户的对话不会出现在这里。</p>
      ...
      <div className="historyStats"><article><strong>{conversations.length}</strong><span>我的历史会话</span></article><article><strong>{messageCount}</strong><span>历史消息</span></article><article><strong>{sessionCount}</strong><span>本机 Session</span></article><article><strong>{sourceCount}</strong><span>来源引用</span></article></div>
    </section>

    <section className="historyPanel">
      <div className="historyToolbar">
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索我的问题、回答或知识库…" />
      ...
      {loading && <div className="historyEmpty">正在读取当前浏览器用户的 AnythingLLM 历史…</div>}
      {error && <div className="historyError"><strong>暂时无法读取历史记录</strong><p>{error}</p></div>}
      {!loading && !error && !filtered.length && <div className="historyEmpty"><strong>当前用户还没有可显示的历史会话。</strong><p>更新到这一版之后的新对话会自动记录 sessionId。为了保护测试用户隐私，旧的全局 AnythingLLM Thread/聊天不会再自动展示。</p></div>}
      ...
      {!loading && !error && filtered.length > 0 && <div className="historyWorkspace">
        <aside className="conversationList">{filtered.map((item) => <button key={item.id} className={selected?.id === item.id ? "active" : ""} onClick={() => setSelectedId(item.id)}>
          <div className="listBadges"><span>{item.workspace}</span><b>我的会话</b></div>
        ...
        <section className="conversationDetail">
          {selected && <>
            <div className="detailHead"><div><div className="detailBadges"><span>{selected.workspace}</span><b>浏览器私有历史</b></div><h2>{selected.title}</h2><small>{formatTime(selected.sentAt)} · {selected.messages.length} 条消息 · {selected.sourceCount} 个来源</small></div><button type="button" onClick={bringBackQuestion}>带回输入框</button></div>
    ...
    <section className="historyNote"><strong>为什么现在看不到以前所有 AnythingLLM Thread？</strong><p>这是有意修改。以前读取“所有 Thread/所有 API 会话”会让不同测试用户看到彼此的历史，不适合作为用户测试 Demo。当前先按本浏览器 sessionId 隔离；等加入账号系统后，再把“真正可续聊的 Thread”绑定到登录用户。</p></section>
